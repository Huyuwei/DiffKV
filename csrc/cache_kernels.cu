...
#define CALL_PROMPT_PHASE_CACHE_LAUNCHER_QUANT_CONFIG(T, HEAD_SIZE)                          \
  ...
  } else if (quant_config == std::vector<int>{8, 8, 8, 4, 1, 1, 1, 2}) {                     \
    assert(num_tokens_per_page_high == 24);                                                  \
    assert(num_tokens_per_page_low == 32);                                                   \
    assert(thread_group_size_v == 8);                                                        \
    LAUNCH_CACHE_KERNEL_PROMPT_PHASE(T, HEAD_SIZE, 8, 4, 8, 2, 1, 1, 1, 2, 24, 32, 8);       \
  } else if (quant_config == std::vector<int>{8, 4, 4, 2, 1, 2, 2, 4}) {                     \
    assert(num_tokens_per_page_high == 32);                                                  \
    assert(num_tokens_per_page_low == 53);                                                   \
    assert(thread_group_size_v == 8);                                                        \
    LAUNCH_CACHE_KERNEL_PROMPT_PHASE(T, HEAD_SIZE, 8, 4, 4, 2, 1, 2, 2, 4, 32, 53, 8);       \
  } else if (quant_config == std::vector<int>{4, 4, 4, 4, 1, 1, 1, 1}) {                     \
    assert(num_tokens_per_page_high == 47);                                                  \
    assert(num_tokens_per_page_low == 47);                                                   \
    assert(thread_group_size_v == 8);                                                        \
    LAUNCH_CACHE_KERNEL_PROMPT_PHASE(T, HEAD_SIZE, 4, 4, 4, 4, 1, 1, 1, 1, 47, 47, 8);       \
  } else if (quant_config == std::vector<int>{4, 2, 4, 2, 2, 4, 2, 4}) {                     \
    assert(num_tokens_per_page_high == 53);                                                  \
    assert(num_tokens_per_page_low == 53);                                                   \
    assert(thread_group_size_v == 8);                                                        \
    LAUNCH_CACHE_KERNEL_PROMPT_PHASE(T, HEAD_SIZE, 4, 2, 4, 2, 2, 4, 2, 4, 53, 53, 8);       \
  } else {                                                                       \
...
#define CALL_DECODE_PHASE_CACHE_LAUNCHER_QUANT_CONFIG(T, HEAD_SIZE)                          \
  if (quant_config == std::vector<int>{8, 8, 8, 8, 1, 1, 1, 1}) {                            \
    ...
    assert(num_tokens_per_page_low == 32);                                                   \
    assert(thread_group_size_v == 8);                                                        \
    LAUNCH_CACHE_KERNEL_DECODE_PHASE(T, HEAD_SIZE, 8, 4, 8, 2, 1, 1, 1, 2, 24, 32, 8);       \
  } else if (quant_config == std::vector<int>{8, 4, 4, 2, 1, 2, 2, 4}) {                     \
    assert(num_tokens_per_page_high == 32);                                                  \
    assert(num_tokens_per_page_low == 53);                                                   \
    assert(thread_group_size_v == 8);                                                        \
    LAUNCH_CACHE_KERNEL_DECODE_PHASE(T, HEAD_SIZE, 8, 4, 4, 2, 1, 2, 2, 4, 32, 53, 8);       \
  } else if (quant_config == std::vector<int>{4, 4, 4, 4, 1, 1, 1, 1}) {                     \
    assert(num_tokens_per_page_high == 47);                                                  \
    assert(num_tokens_per_page_low == 47);                                                   \
    assert(thread_group_size_v == 8);                                                        \
    LAUNCH_CACHE_KERNEL_DECODE_PHASE(T, HEAD_SIZE, 4, 4, 4, 4, 1, 1, 1, 1, 47, 47, 8);       \
  } else if (quant_config == std::vector<int>{4, 2, 4, 2, 2, 4, 2, 4}) {                     \
    assert(num_tokens_per_page_high == 53);                                                  \
    assert(num_tokens_per_page_low == 53);                                                   \
    assert(thread_group_size_v == 8);                                                        \
    LAUNCH_CACHE_KERNEL_DECODE_PHASE(T, HEAD_SIZE, 4, 2, 4, 2, 2, 4, 2, 4, 53, 53, 8);       \
  } else {                                                                       \
    TORCH_CHECK(false, "Unsupported quant config: ", quant_config);              \
  }
