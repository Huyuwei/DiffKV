...
> **NOTE**:
> This `group_quantization` branch introduces an enhanced version of DiffKV that supports quantizing each token with **multiple** sets of metadata, which is potentially more effective for GQA architectures with higher queries-per-KV ratios, as well as for long CoT (Chain-of-Thought) generation. In contrast, the `main` branch of DiffKV quantizes each token (typically with a hidden size of 128) using a **single** set of metadata (scale and zero point). More implementation details can be found in the [Quantization Configuration Guide](#quantization-configuration-guide)
...
- **Parameter:** `self.quantized_kv_bits`\
  ...
  - `n_kgroups` and `n_vgroups` specify how many subgroups each head is split into for quantization metadata (scale & zero point).
  - Up to **4 groups per head** are supported by the current CUDA kernels.
